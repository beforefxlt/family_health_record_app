...
describe('API Constants', () => {
  ...
  describe('METRIC_RANGES', () => {
    ...
    it('should have vision_acuity range 4.0-5.3 (标准对数视力表)', () => {
      expect(METRIC_RANGES.vision_acuity).toEqual({ min: 4.0, max: 5.3, unit: '' });
    });

    it('vision_acuity 范围必须与 API_CONTRACT.md 一致', () => {
      const range = METRIC_RANGES.vision_acuity;
      expect(range.min).toBeGreaterThanOrEqual(4.0);
      expect(range.max).toBeLessThanOrEqual(5.3);
    ...
    it('should have all required metrics', () => {
      const requiredMetrics = ['height', 'weight', 'axial_length', 'vision_acuity', 'glucose', 'ldl', 'hemoglobin', 'hba1c'];
