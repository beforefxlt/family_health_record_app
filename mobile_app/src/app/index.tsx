...
import { testConnection } from '../config/serverConfig';
...
export default function HomeScreen() {
  ...
  const loadMembers = useCallback(async () => {
    try {
      ...
      if (!conn.ok) {
        setConnectionError(conn.message);
        setLoading(false);
        return;
      }
      ...
      setConnectionError(error.message || '无法连接服务器');
    } finally {
      setLoading(false);
    }
  }, []);
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.errorContainer}>
          ...
          <Text style={styles.errorTitle}>无法连接服务器</Text>
          ...
          >
            <Text style={styles.settingsLinkText}>前往设置 → 配置服务器</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
