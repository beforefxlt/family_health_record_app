...
> **最后更新**: 2026-04-12 17:55 by Antigravity
...
| **移动端 APK** | ✅ 模拟器验证通过 | 最新 Release APK 已实现趋势图Y轴上下扩展20%显示，通过透明数据集方案在模拟器验证通过 |
...
### 2.1 核心变更
1. **趋势图Y轴扩展**: 移动端趋势图Y轴范围从最小值下探20%、最大值上探20%，使用透明数据集方案实现，已在模拟器验证。
2. **指标时间展示 (移动端)**: 详情页卡片新增「记录于 YYYY-MM-DD」，完成 MetricCard 重构。
...
| 移动端逻辑测 | 375 | ✅ 全部通过 | 含 Y 轴扩展 20% 回归测试 |
