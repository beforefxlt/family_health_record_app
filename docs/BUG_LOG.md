...
## 📅 v2.9.2 移动端上传流程修复 (2026-04-05)

### [BUG-054] 移动端上传页缺少拍照按钮和确认按钮
- **现象**: 
  - 问题1：只有相册选项，没有拍照按钮
  - 问题2：选择图片后没有确认按钮，流程无法继续
- **根因**: `upload.tsx` 页面 UI 不完整
- **修复**:
  - 新增"拍照"和"相册"两个选项按钮
  - 选择图片后显示"确认上传"按钮
- **验证**: APK 已安装
...
| 指标 | 数值 |
|------|------|
| 新增功能 | 3 个（records列表API、API超时、眼轴年龄参考值） |
| 新增 UT | 6 个（records API 4个 + 眼轴参考值 2个） |
| 修改文件 | 13 个 |
| 测试通过率 | 后端 28 passed / E2E 15 passed |
| Docker 镜像 | v2.5.0 |

---

## 📅 v2.5.0 记录列表与API超时 (2026-04-06)

### 新增功能

- **记录列表 API**: `GET /api/v1/records` - 分页、成员筛选
- **API 超时**: 前端/移动端 30s 超时 + AbortController
- **眼轴参考值**: 按年龄动态计算（21.0~23.5mm）

### 修复的测试问题

- **TC-P1-016**: 修复 `member_type: 'spouse'` 无效枚举值 → 改为 `adult`
